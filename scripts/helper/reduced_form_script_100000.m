...
inputDir = 'F:\Roy\Amazon_data\Mat_files\Books';
outputDir = 'F:\Roy\Amazon_data\Processed_MAT_files\Books';
largeReviewDir = 'F:\Roy\Amazon_data\TooBigReviews\Books';
...
fileNumbers = cellfun(@(x) sscanf(x, 'Books_%d_%*d.mat'), {matFiles.name});
...
for i = 1:length(matFiles)
        ...
        combinedDataStruct(end).num_simultaneous_reviews = counts(idx)';
              ...
              if contains(ME.message, 'exceeds maximum array size preference')
                    ...
                    reviewData  =originalDataStruct(j);
                     ...
                     fprintf('\033[31mSaved large review to %s\033[0m\n', largeReviewFile);
              else
                     rethrow(ME);
              end
              combinedDataStruct = struct('unique_ids', {}, 'asins', {}, 'parent_asins', {}, 'ratings', {}, 'user_ids', {}, 'verified_purchases', {}, 'helpful_votes', {}, 'timestamps', {}, 'images', {}, ...
    ...
    if mod(processedFilesCounter, 10) == 0
        ...
        endIdx = i * chunkSize;
        outputFileName = fullfile(outputDir, sprintf('Books_processed.mat', startIdx, endIdx));
...
if ~isempty(combinedDataStruct)
    ...
    outputFileName = fullfile(outputDir, sprintf('Books_%d_%d_processed.mat', startIdx, endIdx));

...
inputDir = 'F:\Roy\Amazon_data\Mat_files\Clothing_Shoes_and_Jewelry';
outputDir = 'F:\Roy\Amazon_data\Processed_MAT_files\Clothing_Shoes_and_Jewelry';
...
fileNumbers = cellfun(@(x) sscanf(x, 'Clothing_Shoes_and_Jewelry%d_%*d.mat'), {matFiles.name});
...
% *************************************************************************      
        combinedDataStruct(end).asins = originalDataStruct(j).asins(index2NoDuplicates);
        combinedDataStruct(end).parent_asins = originalDataStruct(j).parent_asins(index2NoDuplicates);
        ...
        num_reviews = length(combinedDataStruct(end).ratings);
    ...
    if mod(processedFilesCounter, 10) == 0
        startIdx = (i - 10) * chunkSize + 1;
        endIdx = i * chunkSize;
        outputFileName = fullfile(outputDir, sprintf('Clothing_Shoes_and_Jewelry_%d_%d_processed.mat', startIdx, endIdx));
